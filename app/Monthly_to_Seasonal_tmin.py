...
def Monthly_to_Seasonal_tmin(season, year):
    ...
    st.info(f"Please upload monthly minimum temperature data for all months of the {season} season")
    ...
    if len(dataframes) == 4 and st.button('Convert'):
        ...
        cols = ['No'] + [col for col in seasonal_data if col not in ['No', 'Mean Minimum Temperature']] + ['Mean Minimum Temperature']
        seasonal_data = seasonal_data[cols]

        # Display the seasonal data
        st.info(f"Mean Minimum Temperature for {season} season {year}")
        st.write(seasonal_data)

        st.download_button('Download data as CSV', 
                        seasonal_data.to_csv(index=False),  
                        file_name=(f'tmin_{season}_{year}.csv'), 
                        mime='text/csv', key=f"tmin_seasonal")
        
        st.session_state.seasonal_data = seasonal_data 
        
    seasonal_data = st.session_state.get('seasonal_data', None)    
    st.markdown("---")        
    st.subheader('Seasonal Mean Minimum Temperature Plot')  
    if st.button('Generate Map', key="generate_map_tmin_monthly_to_seasonal"): 
        st.info(f"Mean Minimum Temperature for {season} season {year}")
        center_coordinates = (9.5, 40.5) 
        if seasonal_data is not None:
            seasonal_data = round(seasonal_data,3)
            sea_tmin = px.scatter_mapbox(seasonal_data, lat="Latitude", lon="Longitude", hover_name="Station Name", hover_data=["Elevation", "Year", "Mean Minimum Temperature"],
            color_discrete_sequence=["green"], zoom=5, height=500)
            sea_tmin.update_layout(mapbox_style="open-street-map",
                                margin={"r": 0, "t": 0, "l": 0, "b": 0},
                                mapbox=dict(center=dict(lat=center_coordinates[0], lon=center_coordinates[1]), zoom=5))
            st.plotly_chart(sea_tmin, use_container_width=True)
            
    
    
    st.markdown("---")   
    st.subheader('Seasonal Mean Minimum Temperature  Summary Statistics') 
    if st.button('Generate statistics', key="generate_statistics_tmin_monthly_to_seasonal"):
        tmin_sea_stat = seasonal_data['Mean Minimum Temperature'].describe(include='all') 
        st.info(f"Seasonal mean minimum temperature summary statistics for the year {year}")
        st.write(round(tmin_sea_stat,2)) 

        st.download_button('Download data as CSV', 
                        tmin_sea_stat.to_csv(index=False),  
                        file_name=f'seasonal_tmin_stat_{year}.csv', 
                        mime='text/csv')
                
    #Create a box plot of the 'mean minimum temperature' column
    st.markdown("---") 
    st.subheader('Seasonal Mean Minimum Temperature Box Plot') 
    if st.button('Generate Box Plot', key="90"):
        st.info(f"Seasonal mean minimum temperature box plot for the year {year}")

    # Create a box plot of the 'Seasonal Mean Minimum Temperature' column
        fig_tmin_sea = go.Figure()

        fig_tmin_sea.add_trace(go.Box(
            y=seasonal_data['Mean Minimum Temperature'],
        ...
        fig_tmin_sea.update_layout(
            title='Box Plot of Seasonal Mean Minimum Temperature',
            title_x = 0.4,
            title_y = 0.9,
            yaxis_title='Mean Minimum Temperature',
            #xaxis_title=f'Month',
        ...
        st.plotly_chart(fig_tmin_sea, use_container_width=True)
    

if __name__ == "__main__":
    Monthly_to_Seasonal_tmin()