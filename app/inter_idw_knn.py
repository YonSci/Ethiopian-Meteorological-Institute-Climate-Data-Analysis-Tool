...
def inter_idw_knn():
    ...
    from idw_basic import idw_npoint
    ...
    with col3:
        n_points = st.number_input("Enter the number of points", min_value=1, max_value=30, value=3, step=1, key="n_points")    
        st.info(f'You selected the number of points: {n_points}')
    
    start_time = time.time()
    x_idw_list2, y_idw_list2, z_head2 = idw_npoint(x=lon.astype(float).values.tolist(),
                                               y=lat.astype(float).values.tolist(),
                                               z=zvalue.values.tolist(),
                                               grid_side_length = 100,
                                               n_points=3,
                                               p=p,
                                               rblock_iter_distance=search_radius)
    
    # Create an interpolator function
    f1 = interp2d(x_idw_list2, y_idw_list2, z_head2, kind='cubic')  
    # 'linear', 'cubic', 'quintic'.
    z1_interpolated = f1(lons, lats)
    end_time = time.time()
    time_taken = end_time - start_time
    st.success(f"The KNN IDW process has completed successfully. It took {round(time_taken, 2)} seconds.")
    ...
    # convert to netCDF
    ds_idw_knn = xr.Dataset( data_vars= {var_name:(('Latitude', 'Longitude'),
                                                z1_interpolated), },
    coords={'Latitude':lats, 'Longitude':lons})
    ds_idw_knn[var_name].attrs = {'units':unit, 'long_name':var_name}
    
    ds_idw_knn.Latitude.attrs['units'] = 'degrees_north'
    ds_idw_knn.Latitude.attrs['long_name'] = 'latitude'
    ds_idw_knn.Latitude.attrs['axis'] = 'Y'
    ds_idw_knn.Longitude.attrs['units'] = 'degrees_east'
    ds_idw_knn.Longitude.attrs['long_name'] = 'longitude'
    ds_idw_knn.Longitude.attrs['axis'] = 'X'
    ds_idw_knn.attrs = {'creation_date':datetime.now(), 
    ...
    if st.checkbox("Show the interpolated data", key="_interpolated"):    
            ...
            img = ds_idw_knn[var_name].plot(ax=ax, 
                                    cmap='viridis',
                                    ...
                                    antialiased=True, 
                                    zorder=1,
                                    vmin=ds_idw_knn[var_name].min(), vmax=ds_idw_knn[var_name].max(),
                                    levels=15,
                                    )
    ...
    if ds_idw_knn is None:
        ...
        et_mask = et_poly.mask(ds_idw_knn.Longitude, ds_idw_knn.Latitude)
        
        et_idwmask_knn =  ds_idw_knn.where(et_mask == 0)
        
        min_value = et_idwmask_knn[var_name].min().values
        max_value = et_idwmask_knn[var_name].max().values
        num_ticks = 5
    ...
    if st.checkbox("Show the extracted data", key="_extracted"):
        ...
        c1= et_idwmask_knn[var_name].plot(ax=ax, 
                                cmap='viridis',
                                cbar_kwargs={"shrink": 0.8},
                                transform=ccrs.PlateCarree(), 
                                extend='both', 
                                ...
                                antialiased=True, 
                                zorder=1, 
                                vmin=et_idwmask_knn[var_name].min(), 
                                vmax=et_idwmask_knn[var_name].max(),
                                levels=12)
    ...
    if clicked:  
        et_idwmask_knn.attrs['creation_date'] = et_idwmask_knn.attrs['creation_date'].isoformat()
        et_idwmask_knn.to_netcdf(f'ds_{variable}_masked_idwknn.nc', engine='h5netcdf')

        with open(f"ds_{variable}_masked_idwknn.nc", "rb") as fp:
            btn = st.download_button(
                label="Download netCDF file",
                data=fp,
                file_name=f"ds_{variable}_masked_idwknn.nc",
            )
