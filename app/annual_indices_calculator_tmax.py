...
def annual_indices_calculator_tmax(variable_type, an_max):
    ...
    if an_max is not None:
        max_tmax = an_max.loc[:, 'Mean Maximum Temperature':'Mean Maximum Temperature'].max().max()
        min_tmax = an_max.loc[:, 'Mean Maximum Temperature':'Mean Maximum Temperature'].min().min()
        ...
        tmax = slider_placeholder.slider('Select maximum temprature values:', int(min_tmax), int(max_tmax), 5, key="tmax_slider1")
        ...
        df_max_temp_stations = an_max.loc[:,['Station Name', 'Longitude', 'Latitude', 'Elevation', 'ELID', 'Year']] 
        df_max_temp_dates = an_max.loc[:,'Mean Maximum Temperature':'Mean Maximum Temperature']
        df_max_temp_dates_filtered = df_max_temp_dates[(df_max_temp_dates >= tmax)]
        
        
        dict_max_temp_reports = {'Station Name': [], 'Lon': [], 'Lat': [], 'Elev': [], 
                                 'ELID': [], 'Year': [],
        ...
        st.download_button('Download data as CSV', 
                            ...
                            file_name=f'df_rf_{tmax}_{year}.csv', 
                            mime='text/csv')
        ...
        st.info(f'Stations that exceeds {tmax} °C in the year {year}')
        ...
        fig_tmax_i1 = px.scatter_mapbox(df_max_temp_reports, 
                                    ...
                                    hover_data=["Elevation", "Year", "Longitude", "Latitude","Mean Maximum Temperature (°C)"],
                                    height=600,
                                    width=800
                                    )
...
if __name__ == "__main__":
    annual_indices_calculator_tmax()