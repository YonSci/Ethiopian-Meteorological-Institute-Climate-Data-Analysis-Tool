...
def monthly_data_miss_tmean(variable_type, mon_mean):
    ...
    if variable_type == 'Mean Temperature':
        ...
        if st.button('Count Missing Data: Mean Temperature', key="mon_miss_tmean1"):
            if mon_mean is None:
                st.warning('Please Load Data in Data Importing Page 😔')
            else:
                missing_data_count_tmean = mon_mean.iloc[:, 8:9].isnull().sum().sum()
                
                st.info(f'Missing mean temperature data count in {month} {year} is **_{missing_data_count_tmean}_**')
                total_data_points_tmean = mon_mean.iloc[:, 8:9].size
                percentage_missing_tmean = (missing_data_count_tmean / total_data_points_tmean) * 100
                st.info(f'The percentage of missing mean temperature data in {month} {year} is **_{round(percentage_missing_tmean, 2)}%_**')
                            
        
        st.markdown("---")
        st.subheader('Missing data counts by station')
        if st.button('Tabular Report: Mean Temperature', key="mon_miss_tmean2"):
            if mon_mean is None:
                st.warning('Please Load Data in Data Importing Page 😔')
            else:
                missingdata_row_tmean = mon_mean.iloc[:, 8:9].isnull().sum(axis=1)        
                ...
                sub_tmean = mon_mean.iloc[:, 0:8]                
                sub_tmean_df = pd.DataFrame(sub_tmean)
                tmean_join = sub_tmean_df.join(missingdata_tmean_df)                         
                tmean_miss_st = tmean_join[["No", "Gh id", "Station Name", "Latitude", "Longitude", "Elevation", "Year", "Month", "Missing_Data_Count"]]
        
                st.info(f'Missing mean temperature data counts for {month} {year}')
                st.write(tmean_miss_st)
                st.download_button('Download data as CSV', 
                                tmean_miss_st.to_csv(index=False),  
                                file_name=(f'tmean_missing_data_by_station_{month}_{year}_{month}.csv'), 
                                mime='text/csv', key="mon_miss3")
                
        st.markdown("---")
        st.subheader('Stations Without Missing Data')
        if st.button('Tabular Report: Mean Temperature', key="mon_miss4"):
            if mon_mean is None:
                st.warning('Please Load Data in Data Importing Page 😔')
            else:
                missingdata_row_tmean = mon_mean.iloc[:, 8:9].isnull().sum(axis=1)
                ...
                st.download_button('Download data as CSV', 
                                ...
                                file_name=(f'tmean_stationsless1_missing_{month}_{year}.csv'), 
                                mime='text/csv', key="mon_miss5")        
            
        if st.button('Generate Map: Mean Temperature', key="mon_miss6"):
            if mon_mean is None:
                st.warning('Please Load Data in Data Importing Page 😔')
            else:
                missingdata_row_tmean = mon_mean.iloc[:, 8:9].isnull().sum(axis=1)
                ...
                names_tmean[names_tmean.index(0)] = 'Missing_Data_Count'
                missingdata_tmean_df.columns = names_tmean
                sub_tmean = mon_mean.iloc[:, 0:9]
                sub_tmean_df = pd.DataFrame(sub_tmean)
                tmean_join = sub_tmean_df.join(missingdata_tmean_df)
                tmean_miss_st = tmean_join[["No", "Gh id", "Station Name", "Latitude", "Longitude", "Elevation", "Year", "Month", "Mean Temperature", "Missing_Data_Count"]]
                ...
                fig2_tmean_f2 = px.scatter_mapbox(miss_tmean3, lat="Latitude", lon="Longitude", 
                                            ...
                                            hover_data=["Elevation", "Month", "Year", "Missing_Data_Count", "Mean Temperature"],
