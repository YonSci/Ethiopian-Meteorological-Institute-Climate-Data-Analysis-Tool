...
def annual_indices_calculator_tmin(variable_type, an_min):
    ...
    if an_min is not None:
        max_tmin = an_min.loc[:, 'Mean Minimum Temperature':'Mean Minimum Temperature'].max().max()
        min_tmin = an_min.loc[:, 'Mean Minimum Temperature':'Mean Minimum Temperature'].min().min()
        ...
        df_min_temp_stations = an_min.loc[:,['Station Name', 'Longitude', 'Latitude', 'Elevation', 'ELID', 'Year']] 
        df_min_temp_dates = an_min.loc[:,'Mean Minimum Temperature':'Mean Minimum Temperature']
        df_min_temp_dates_filtered = df_min_temp_dates[(df_min_temp_dates <= tmin)]
        
        
        dict_min_temp_reports = {'Station Name': [], 'Lon': [], 'Lat': [], 'Elev': [], 'ELID': [], 
                                 'Year': [], 'Mean Minimum Temperature (°C)': []}
        ...
        st.download_button('Download data as CSV', 
                            ...
                            file_name=f'df_rf_{tmin}_{year}.csv', 
                            mime='text/csv')
        ...
        st.info(f'Stations below {tmin} °C in the year {year}')
        ...
        fig_tmin_i1 = px.scatter_mapbox(df_min_temp_reports, 
                                    ...
                                    hover_data=["Elevation", "Year", "Longitude", "Latitude","Mean Minimum Temperature (°C)"],
                                    height=600,
                                    width=800
                                    )
...
if __name__ == "__main__":
    annual_indices_calculator_tmin()