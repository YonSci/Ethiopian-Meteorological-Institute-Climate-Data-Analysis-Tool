...
# Mean Temperature Monthly to Annual Conversion
def Monthly_to_Annual_tmean(year):
    year = st.session_state.get('year', None)
    st.markdown("---")        

    st.info(f"Please upload monthly mean temperature data for all months of the year {year}")
    ...
    if st.button('Generate Map', key="generate_map_tmean_monthly_to_annual"): 
        st.info(f"Annual mean temperature for the year {year}")
    ...
    if st.button('Generate statistics', key="generate_statistics_tmean_monthly_to_annual"):
        rainfall_ann_stat = annual_data['Mean Temperature'].describe(include='all') 
        st.info(f"Annual mean temperature summary statistics for the year {year}")
        rainfall_ann_stat = round(rainfall_ann_stat, 3)
