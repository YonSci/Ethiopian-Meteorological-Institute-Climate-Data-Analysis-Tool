...
def monthly_indices_calculator_tmax(variable_type, mon_max):
    ...
    if mon_max is not None:
        max_tmax = mon_max.loc[:, 'Mean Maximum Temperature':'Mean Maximum Temperature'].max().max()
        min_tmax = mon_max.loc[:, 'Mean Maximum Temperature':'Mean Maximum Temperature'].min().min()
        ...
        df_max_temp_stations = mon_max.loc[:,['Station Name', 'Longitude', 'Latitude', 'Elevation', 'ELID', 'Year', 'Month']] 
        df_max_temp_dates = mon_max.loc[:,'Mean Maximum Temperature':'Mean Maximum Temperature']
        ...
        st.download_button('Download data as CSV', 
                            ...
                            file_name=f'df_rf_{tmax}_{month}_{year}.csv', 
                            mime='text/csv')
        ...
        fig_tmax_i1 = px.scatter_mapbox(df_max_temp_reports, 
                                    ...
                                    hover_data=["Elevation", "Month", "Year", "Longitude", "Latitude","Mean Maximum Temperature (°C)"],
                                    height=600,
                                    width=800
                                    )
...
if __name__ == "__main__":
    monthly_indices_calculator_tmax()