...
def Monthly_to_Seasonal_rf(season, year):
    ...
    st.info(f"Please upload monthly rainfall data for all months of the {season} season")
    ...
    if len(dataframes) == 4 and st.button('Convert'):
        ...
        cols = ['No'] + [col for col in seasonal_data if col not in ['No', 'Total Rainfall']] + ['Total Rainfall']
        seasonal_data = seasonal_data[cols]

        # Display the seasonal data
        st.info(f" Total rainfall for {season} season {year}")
        st.write(seasonal_data)

        st.download_button('Download data as CSV', 
                        seasonal_data.to_csv(index=False),  
                        file_name=(f'rf_{season}_{year}.csv'), 
                        mime='text/csv', key=f"rf_seasonal")
        
        st.session_state.seasonal_data = seasonal_data 
        
    seasonal_data = st.session_state.get('seasonal_data', None)    
    st.markdown("---")        
    st.subheader('Seasonal Total Rainfall Plot')  
    if st.button('Generate Map', key="generate_map_rf_monthly_to_seasonal"): 
        st.info(f" Total rainfall for {season} season {year}")
        center_coordinates = (9.5, 40.5) 
        if seasonal_data is not None:
            seasonal_data = round(seasonal_data,3)
            sea_rf = px.scatter_mapbox(seasonal_data, lat="Latitude", lon="Longitude", hover_name="Station Name", hover_data=["Elevation", "Year", "Total Rainfall"],
            color_discrete_sequence=["green"], zoom=5, height=500)
            sea_rf.update_layout(mapbox_style="open-street-map",
                                margin={"r": 0, "t": 0, "l": 0, "b": 0},
                                mapbox=dict(center=dict(lat=center_coordinates[0], lon=center_coordinates[1]), zoom=5))
            st.plotly_chart(sea_rf, use_container_width=True)
            
    
    
    st.markdown("---")   
    st.subheader('Seasonal Rainfall Summary Statistics') 
    if st.button('Generate statistics', key="generate_statistics_rf_monthly_to_seasonal"):
        rainfall_sea_stat = seasonal_data['Total Rainfall'].describe(include='all') 
        st.info(f"Seasonal total rainfall summary statistics for the year {year}")
        st.write(round(rainfall_sea_stat,2)) 

        st.download_button('Download data as CSV', 
                        rainfall_sea_stat.to_csv(index=False),  
                        file_name=f'seasonal_total_rf_stat_{year}.csv', 
                        mime='text/csv')
                
    #Create a box plot of the 'Seasonal Total Rainfall' column
    st.markdown("---") 
    st.subheader('Seasonal Total Rainfall Box Plot') 
    if st.button('Generate Box Plot', key="90"):
        st.info(f"Seasonal total rainfall box plot for the year {year}")

    # Create a box plot of the 'Seasonal Total Rainfall' column
        fig_rf_sea = go.Figure()

        fig_rf_sea.add_trace(go.Box(
            y=seasonal_data['Total Rainfall'],
            name='Total Rainfall',
            marker_color='blue',
        ...
        fig_rf_sea.update_layout(
            title='Box Plot of Seasonal Total Rainfall ',
            title_x = 0.4,
            title_y = 0.9,
        ...
        st.plotly_chart(fig_rf_sea, use_container_width=True)
    

if __name__ == "__main__":
    Monthly_to_Seasonal_rf()