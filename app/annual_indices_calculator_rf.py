...
def annual_indices_calculator_rf(variable_type, an_rf):
    ...
    if an_rf is not None:
        max_rf = an_rf.loc[:, 'Total Rainfall':'Total Rainfall'].max().max()
        min_rf = an_rf.loc[:, 'Total Rainfall':'Total Rainfall'].min().min()
        ...
        df_rainfall_stations = an_rf.loc[:,['Station Name', 'Longitude', 'Latitude', 'Elevation', 'ELID', 'Year']] 
        df_rainfall_dates = an_rf.loc[:,'Total Rainfall':'Total Rainfall']
        df_rainfall_dates_filtered = df_rainfall_dates[(df_rainfall_dates >= rf)]
        
        
        dict_rainfall_reports = {'Station Name': [], 'Lon': [], 'Lat': [], 'Elev': [], 
                                 'ELID': [], 
                                 'Year': [], 'Rain Fall (mm)': []}
        ...
        st.download_button('Download data as CSV', 
                            ...
                            file_name=f'df_rf_{rf}_{year}.csv', 
                            mime='text/csv')
        ...
        st.info(f'Stations that exceeds {rf} mm of rainfall in the year {year}')
        st.plotly_chart(fig)
        ...
        fig_rf_i1 = px.scatter_mapbox(df_rainfall_reports, 
                                    ...
                                    hover_data=["Elevation", "Year", "Longitude", "Latitude","Rain Fall (mm)"],
                                    height=600,
                                    width=800
                                    )
        ...
        st.error("No annual rainfall data provided.")
        
        
if __name__ == "__main__":
    annual_indices_calculator_rf()