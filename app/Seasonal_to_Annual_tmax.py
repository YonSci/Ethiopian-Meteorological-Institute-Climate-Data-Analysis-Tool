...
def Seasonal_to_Annual_tmax(year):
    year = st.session_state.get('year', None)
    st.markdown("---")    
    
    st.info(f"Please upload seasonal mean maximum temperature data for all seasons of the year {year}")
    ...
    if st.button('Generate Map', key="generate_map_tmax_seasonal_to_annual"): 
        st.info(f"Annual mean maximum temperature for the year {year}")
    ...
    if st.button('Generate statistics', key="generate_statistics_tmax_seasonal_to_annual"):
        tmax_ann_stat = annual_data['Mean Maximum Temperature'].describe(include='all') 
        st.info(f"Annual mean maximum temperature summary statistics for the year {year}")
        st.write(round(tmax_ann_stat,2)) 

        st.download_button('Download data as CSV', 
                        tmax_ann_stat.to_csv(index=False),  
