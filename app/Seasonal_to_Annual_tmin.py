...
def Seasonal_to_Annual_tmin(year):
    year = st.session_state.get('year', None)
    st.markdown("---")    
    
    st.info(f"Please upload seasonal mean minimum temperature data for all seasons of the year {year}")
    ...
    if st.button('Generate Map', key="generate_map_tmin_seasonal_to_annual"): 
        st.info(f"Annual mean minimum temperature for the year {year}")
    ...
    if st.button('Generate statistics', key="generate_statistics_tmin_seasonal_to_annual"):
        tmin_ann_stat = annual_data['Mean Minimum Temperature'].describe(include='all') 
        st.info(f"Annual mean minimum temperature summary statistics for the year {year}")
        st.write(round(tmin_ann_stat,2)) 

        st.download_button('Download data as CSV', 
                        tmin_ann_stat.to_csv(index=False),  
