...
def annual_maximum_temperature():
    ...
    if plot_type == 'Mean maximum temperature':
        ...
        st.markdown("#### :blue[Upload annual maximum temperature NetCDF File]" )
        ...
        st.markdown("#### :blue[Display Annual Mean Maximum Temperature]" )  
        if ds_temp_max is not None:
            if st.checkbox("Plot Annual Mean Maximum Temperature", key="temp_display"):
                ...
                # Add a title
                ax.set_title(f'{variable} in the year {year}', fontsize=20, y=1.01, fontname=title_font)
