...
def annual_rainfall():
    ...
    if plot_type == 'Annual Total Rainfall':
        ...
        if ds is not None:
            if st.checkbox("Plot Annual Rainfall Data", key="_display"):
                ...
                # Add a title
                ax.set_title(f'Annual {variable} during {year}', fontsize=20, y=1.01, fontname=title_font)
    ...
    if plot_type == 'Departure of Annual Total Rainfall':
        ...
        if var4 is not None and var5 is not None:
            ...
            if ds is not None:
                if st.checkbox("Plot Annual Departure of Rainfall", key="_display"):
                    fig, ax = plt.subplots(figsize=(12, 8), 
                                            subplot_kw={'projection': ccrs.PlateCarree()},
                                            #facecolor=bg_color,
                                            edgecolor='black')
                    ...
                    # Add a title
                    ax.set_title(f'Departure of {year} rainfall from previous year', fontsize=20, y=1.01, fontname=title_font)
