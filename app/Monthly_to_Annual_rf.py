...
def Monthly_to_Annual_rf(year):
    year = st.session_state.get('year', None)
    st.markdown("---")        

    st.info(f"Please upload monthly rainfall data for all months of the year {year}")
    ...
    if st.button('Generate Map', key="generate_map_rf_monthly_to_annual"): 
        st.info(f"Annual total rainfall for the year {year}")
