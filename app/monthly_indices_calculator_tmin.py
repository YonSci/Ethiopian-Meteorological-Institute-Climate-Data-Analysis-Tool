...
def monthly_indices_calculator_tmin(variable_type, mon_min):
    ...
    if mon_min is not None:
        max_tmin = mon_min.loc[:, 'Mean Minimum Temperature':'Mean Minimum Temperature'].max().max()
        min_tmin = mon_min.loc[:, 'Mean Minimum Temperature':'Mean Minimum Temperature'].min().min()
        ...
        df_min_temp_stations = mon_min.loc[:,['Station Name', 'Longitude', 'Latitude', 'Elevation', 'ELID', 'Year', 'Month']] 
        df_min_temp_dates = mon_min.loc[:,'Mean Minimum Temperature':'Mean Minimum Temperature']
        ...
        dict_min_temp_reports = {'Station Name': [], 'Lon': [], 'Lat': [], 'Elev': [], 'ELID': [], 'Year': [],
                                'Month': [], 'Mean Minimum Temperature (°C)': []}
        ...
        st.download_button('Download data as CSV', 
                            ...
                            file_name=f'df_rf_{tmin}_{month}_{year}.csv', 
                            mime='text/csv')
        ...
        fig_tmin_i1 = px.scatter_mapbox(df_min_temp_reports, 
                                    ...
                                    hover_data=["Elevation", "Month", "Year", "Longitude", "Latitude","Mean Minimum Temperature (°C)"],
                                    height=600,
                                    width=800
                                    )
...
if __name__ == "__main__":
    monthly_indices_calculator_tmin()